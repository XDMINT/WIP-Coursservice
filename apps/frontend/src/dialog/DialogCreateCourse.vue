<template>
  <v-dialog v-model="courseDialog" width="70%">
    ...
    <v-snackbar v-model="snackbarFail" :timeout="3500">{{ snackbarFailMessage }}</v-snackbar>
  </v-dialog>
...
const snackbarFailMessage = ref('Kurs konnte nicht gespeichert werden, bitte versuchen Sie es erneut.')
...
const openDialog = (courseId: string | number | undefined) => {
  loading.value = false
  initializeSemesters()
...
const initializeDialogForEditingCourse = (courseId: string | number) => {
