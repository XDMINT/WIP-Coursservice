<template>
  ...
  <div class="course-members">
    <v-card>
      <v-card-title class="task-header-title">
        ...
        <v-spacer />
        <v-btn variant="text" icon="mdi-keyboard-return" color="primary" @click="returnToCourse" />
      </v-card-title>
      <v-card-text>
        <p>{{ courseName }}</p>
        ...
        <div class="align-items-center">
          <v-chip prepend-icon="mdi-account-circle" color="secondary" label>
            {{ courseRole }}
          </v-chip>
          <v-spacer />
          <v-btn v-if="courseRole == 'TEACHER' || courseRole == 'TUTOR'" variant="text" color="error" @click="kickAllStudents"> Alle Studenten entfernen </v-btn>
        </div>
      </v-card-text>
    </v-card>
    ...
    <div class="container">
      ...
      <v-data-table :headers="headers" :items="members" item-value="name" class="elevation-1" :search="search" density="default" height="480px">
        <template #[`item.role`]="{ item }">
          <v-select v-model="item.role" item variant="plain" :items="['STUDENT', 'TUTOR', 'TEACHER']" class="select" :disabled="courseRole == 'STUDENT'" @update:model-value="changeRole(item)" />
        </template>
        <template #[`item.actions`]="{ item }">
          <v-btn icon="mdi-delete" color="error" variant="text" :disabled="courseRole == 'STUDENT'" @click="kickUser(item.user)" />
        </template>
      </v-data-table>
    </div>
  ...
  <v-snackbar v-model="snackbarError" :timeout="3500">{{ snackbarErrorMessage }}</v-snackbar>
</template>
...
const changeRole = (member: Member) => {
  courseService
    ...
    .catch(() => {
      showSnackbarError('Rolle konnte nicht geändert werden.')
      loadCourseMembers()
    })
}
