...
describe('CoursesService course catalog, enrollment and versioning', () => {
  it('separates enrolled courses from published enrollable courses', async () => {
    ...
    const enrolledCourses = await service.getEnrolledCourses('3');
    ...
    await expect(service.getEnrolledCourses('1')).resolves.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: 'managed-course',
          membershipRole: CourseMemberRole.TEACHER,
          currentRun: expect.objectContaining({
            enrollmentCount: 1,
            materialCount: 1,
            taskCount: 1,
