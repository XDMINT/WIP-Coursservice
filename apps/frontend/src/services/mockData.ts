...
type DemoDatabase = {
  semesters: Semester[]
  courses: CoursePL[]
  membersByCourseId: Record<string, Member[]>
}
...
const createMember = (user: User, role: string): Member => ({
  user: {
    ...
    lastName: role === CourseRoles.TEACHER ? 'Lehrperson' : role === CourseRoles.TUTOR ? 'Tutor' : 'Student',
...
const createDefaultDatabase = (): DemoDatabase => {
  ...
  return {
    ...
    courses,
    membersByCourseId: {
      1: [createMember(demoUsers.admin, CourseRoles.TEACHER), createMember(demoUsers.tutor, CourseRoles.TUTOR), createMember(demoUsers.student, CourseRoles.STUDENT)],
      2: [createMember(demoUsers.tutor, CourseRoles.TEACHER)],
      3: [createMember(demoUsers.admin, CourseRoles.TEACHER), createMember(demoUsers.student, CourseRoles.STUDENT)]
    }
  }
}
...
const nextCourseId = () => Math.max(...database.courses.map((course) => Number(course.id)), 0) + 1
...
export const getCourseById = (courseId: string | number): CoursePL | undefined => clone(database.courses.find((course) => course.id === courseId))
...
export const upsertCourse = (course: CoursePL, ownerId: number): CoursePL => {
  ...
  const savedCourse = {
    ...clone(course),
    id: course.id && Number(course.id) > 0 ? course.id : nextCourseId(),
    owner: ownerId,
  ...
  if (existingIndex >= 0) {
    ...
    database.membersByCourseId[savedCourse.id] = [createMember(demoUsers.admin.id === ownerId ? demoUsers.admin : ownerId === demoUsers.tutor.id ? demoUsers.tutor : demoUsers.student, CourseRoles.TEACHER)]
  }
...
export const removeCourse = (courseId: string | number) => {
  database.courses = database.courses.filter((course) => course.id !== courseId)
  delete database.membersByCourseId[courseId]
  persist()
}

export const joinCourse = (courseId: string | number, key: string, userId: number) => {
...
export const leaveCourse = (courseId: string | number, userId?: number) => {
  if (userId == null) {
    return
  }

  const memberList = database.membersByCourseId[courseId] ?? []
  database.membersByCourseId[courseId] = memberList.filter((member) => member.user.id !== userId || member.role === CourseRoles.TEACHER)
  persist()
}

export const getUserRoleInCourse = (userId: number, courseId: string | number) => {
  const course = database.courses.find((entry) => entry.id === courseId)
...
export const changeUserRole = (courseId: string | number, userId: number, role: string) => {
  const memberList = database.membersByCourseId[courseId] ?? []
...
export const getCourseMembers = (courseId: string | number): Member[] => clone(database.membersByCourseId[courseId] ?? [])
